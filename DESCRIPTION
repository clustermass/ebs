...
backup timestamp with its backupPeriod. Due VMs are processed sequentially.
...
the share. Once the queue is empty, configured NAS devices are shut down.
Shutdown is deduplicated by IP for multiple shares on one physical NAS.
...
as a fallback. A suspended VM is either treated as an error or silently skipped,
depending on its configuration. Export begins only after ESXi reports that the
VM is powered off.
...
- temp_process_machine.log carries worker detail back to the coordinator.
...
- process_machine.sh: VM discovery, power handling, export, and promotion.
- mount_share.sh / unmount_share.sh: per-backup CIFS handling.
...
- bin/ovftool/: Git-ignored location for a separately licensed local VMware OVF
  Tool installation; EBS also accepts ovftool on PATH.
...
- A suppressed suspended VM is reported as successful without a new export.
